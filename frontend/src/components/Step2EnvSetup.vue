    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Creating a new simulation instance and retrieving the simulation environment template.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Automatically analyze entities and relationships from the knowledge graph, initialize simulation agents, and assign them unique personas and memories based on real-world triggers.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Initial Trigger Topics</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets simulation time flow, recommendation algorithms, active periods for individuals, posting frequency, and event triggers based on simulation requirements.
          </p>
      ...
      <!-- Step 04: Initial Activation Sequence -->
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on the narrative direction, automatically generate initial activation events and hot topics to guide the simulation world's initial state.
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish automatically planned <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real-world progression. Each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>For 100 agents: Estimated duration approx. {{ Math.round(customMaxRounds * 0.6) }} mins</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        For 100 agents: Estimated duration {{ Math.round(autoGeneratedRounds * 0.6) }} mins
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">First time? We strongly recommend 'Custom mode' to reduce rounds for a quick preview ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">Narrative Experience</span>
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Memories formed based on triggers</span>
              </div>
              <div class="dimension-card">
...
// Calculate the number of automatically generated rounds from the configuration (without using hardcoded defaults)）
...
// according toagent_idGet the correspondingusername
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User-defined number of rounds, passed max_rounds parameter
    params.maxRounds = customMaxRounds.value
    addLog(`Start simulation and customize the number of rounds: ${customMaxRounds.value} wheel`)
  } else {
    // User chooses to keep the automatically generated number of rounds and not pass max_rounds parameter
    addLog(`Start simulation, using automatic configuration of number of rounds: ${autoGeneratedRounds.value} wheel`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`Read from the Zep spectrum ${res.data.expected_entities_count} entities`)
      ...
      addLog(`prepare to fail: ${res.error || 'unknown error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        addLog(`✗ prepare to fail: ${data.error || 'unknown error'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ AgentPersonality ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'Unknown occupation'})`)
        
        // If all generation is completed
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ all ${currentCount} Agent profile generation completed`)
        }
      }
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          addLog(`  ├─ Agentquantity: ${data.summary.total_agents}indivual`)
          addLog(`  ├─ Simulation duration: ${data.summary.simulation_hours}Hour`)
          addLog(`  ├─ initial post: ${data.summary.initial_posts_count}strip`)
          addLog(`  ├─ hot topics: ${data.summary.hot_topics_count}indivual`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time configuration: every round${tc.minutes_per_round}minute, common${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}wheel`)
        }
...
const loadPreparedData = async () => {
  ...
  addLog(`Loaded ${profiles.value.length} Agent profile`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Show detailed configuration summary
        if (res.data.summary) {
          addLog(`  ├─ Agentquantity: ${res.data.summary.total_agents}indivual`)
          addLog(`  ├─ Simulation duration: ${res.data.summary.simulation_hours}Hour`)
          addLog(`  └─ initial post: ${res.data.summary.initial_posts_count}strip`)
        }
