...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // user icon - representative dialogue
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // lightning icon - stands for fast
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // chart icon - representative statistics
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Database icon - Represent entity
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - match"Relevant prediction facts: Xstrip"Format
    ...
    // Extract subproblems - Complete extraction, no limit on quantity
    ...
    // Extract core entities - Full extraction, including summary and relevant fact count
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract history/expired fact - Complete extraction, no limit on quantity
    const histSection = text.match(/### 【history\/expired fact】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const topicMatch = text.match(/\*\*Interview topics:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Extract the number of people interviewed (e.g. "5 / 9 bit simulationAgent"）
    const countMatch = text.match(/\*\*Number of interviews:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Extract reasons for selecting interviewees
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract answers - divided into Twitter andReddit
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatible with old format: case where there is only one platform tag）
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/turn up\s*(\d+)\s*strip/)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `subproblem (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `common ${props.result.facts.length} strip`)
          ]),
          ...
          }, expandedFacts.value ? `close ▲` : `Expand all ${props.result.facts.length} strip ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `common ${props.result.entities.length} indivual`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount}strip`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `close ▲` : `Expand all ${props.result.entities.length} indivual ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `common ${props.result.relations.length} strip`)
          ]),
          ...
          }, expandedRelations.value ? `close ▲` : `Expand all ${props.result.relations.length} strip ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift query generation analysis subproblem'),
            h('span', { class: 'panel-count' }, `common ${props.result.subQueries.length} indivual`)
          ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No core entity yet'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relationship link yet')
      ])
    ])
  }
}
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `common ${props.result.activeFacts.length} strip`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'There is currently no valid memory'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `close ▲` : `Expand all ${props.result.activeFacts.length} strip ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `common ${props.result.historicalFacts.length} strip`)
          ]),
          ...
          }, expandedHistorical.value ? `close ▲` : `Expand all ${props.result.historicalFacts.length} strip ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 2. "1. " or "\n1. " （number+Click, old format compatible）
      let matches = []
      let match
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual platform switch button (only displayed when there is a real dual platform answer）
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      showTabs.value && h('div', { class: 'quicksearch-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `fact (${props.result.facts.length})`)
        ]),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `common ${props.result.edges.length} strip`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `common ${props.result.nodes.length} indivual`)
          ]),
...
const activeStep = computed(() => {
  ...
  return steps[0] || { noLabel: '--', title: 'Waiting to begin', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Handle newlines - Blank lines become paragraph breaks, single line breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // clean up <p><br> Following block-level elements (resulting from extra blank lines）
