...
const runStatus = ref({})
const allActions = ref([]) // All actions (incremental cumulative）
const actionIds = ref(new Set()) // Action ID collection used for deduplication
const scrollContainer = ref(null)

// Computed
// Show actions in chronological order (newest at the end, i.e. at the bottom）
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Set the maximum number of simulation rounds: ${props.maxRounds}`)
    }
    
    addLog('Dynamic map update mode has been turned on')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleaned old simulation logs, restarted simulation')
      }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detects whether the simulation has completed (via runner_status Or the platform completes the status judgment）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: If the backend hasn’t been updated yet runner_status，But the platform has reported completion
      // Pass detection twitter_completed and reddit_completed Status judgment
    ...
    console.warn('Failed to obtain running status:', err)
  }
}
...
const checkPlatformsCompleted = (data) => {
  ...
  // pass actions_count Determine whether the platform is enabled (if count > 0 or running was true）
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // use all_actions Get the full list of actions
