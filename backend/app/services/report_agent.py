...
class ReportLogger:
    ...
    Each row is a complete JSON object, including timestamp, action type, detailed content, etc.。
    """
    ...
    def log_section_content(
        ...
        """Record the chapter content generation is completed (only record the content, does not mean the entire chapter is completed）"""
        self.log(
...
class ReportConsoleLogger:
    ...
    Write console-style logs (INFO, WARNING, etc.) console_log.txt document.
    These logs are related to agent_log.jsonl Different, it is console output in plain text format.。
    """
...
2. Intelligent selection of agents most relevant to the interview topic (such as students, media, officials, etc.)）
...
[Usage scenarios】
- It is necessary to understand the views of events from the perspectives of different characters (what do students think? What do the media think? What do officials say？）
...
- To make your report more vivid, include"Interview transcript"

【Return content】
- Identity information of the agent being interviewed
- Interview responses from each agent on Twitter and Reddit
...
【Core Concept】
We build a simulation world and inject specific "simulation requirements" as variables into it. The evolution of the simulated world is a prediction of what may happen in the future. What you are observing is not"experimental data"，Rather"Preview of the future"。

【Your mission]
Write a "Future Forecast Report" and answer：
1. What happens in the future under the conditions we set？
2. How various types of Agents (people) react and act？
...
- Content should be refined and focused on core predictions and findings
...
【Predictive scenario settings]
The variables we inject into the simulation world (simulation requirements）：{simulation_requirement}
...
Please look at this preview of the future from a "God's perspective"：
1. What will the future look like under the conditions we set?？
2. How various groups of people (Agents) react and act？
3. What future trends does this simulation reveal that are worth watching?
...
Forecasting scenarios (simulating demand）: {simulation_requirement}
...
- Discover future trends, risks and opportunities worth watching
...
2. 【The original words and deeds of the Agent must be quoted】
   - Agentspeech and behavior are predictions of future crowd behavior
   - Present these predictions in reports using a citation format, e.g.：
...
3. 【language consistency - Quotes must be translated into the reporting language】
   ...
   - If the original texts of the simulation requirements and materials are in Chinese, the report must be entirely written in Chinese.
...
- ✅ use**Bold**、Use paragraph breaks, quotes, lists to organize content, but no headings
...
═══════════════════════════════════════════════════════════════
【Available search tools] (called for each chapter3-5Second-rate）
═══════════════════════════════════════════════════════════════

{tools_description}

【Tool usage suggestions - Use a mix of tools, don’t just use one】
...
OptionsA - Call tool:
Output your thinking and then call a tool using the following format：
<tool_call>
...
- It is forbidden to make up your own tools to return results (Observation). All tool results are injected by the system.
...
3. Use Markdown format (but disallow titles)）：
...
4. 【Citation format specifications - Must be divided into separate sections]
   The quotation must be an independent paragraph, with a blank line before and after it, and cannot be mixed in the paragraph.：
...
3. After collecting enough information, output the Final Answer (plain text, without any titles)）"""
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "【Note] You only called{tool_calls_count}tools, which require at least{min_tool_calls}Second-rate。"
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately use the information you have obtained to "Final Answer:" Output chapter content at the beginning。'
)
...
2. Answer questions directly and avoid lengthy reflections
3. Only call the tool to retrieve more data if the report content is insufficient to answer the question
4. Answers should be concise, clear, and organized

[Available Tools] (Only used when needed, call up to1-2Second-rate）
{tools_description}
...
class ReportAgent:
    ...
    adoptReACT（Reasoning + Acting）model：
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or need description (e.g.：'Understand students’ views on the formaldehyde incident in dormitories'）",
                    "max_agents": "The maximum number of agents to interview (optional, default 5, maximum10）"
                }
            }
        }
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        Args:
            ...
            report_context: reporting context (forInsightForge）
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # in-depth interview - Call the real OASIS interview API to obtain the simulated Agent’s answers (dual platform）
            ...
            # ========== Backward compatibility with old tools (internal redirection to new tools） ==========
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        2. Naked JSON (response as a whole or a single line is a tool call JSON）
        """
        tool_calls = []
        ...
        # Format2: reveal all the details - LLM Directly output bare JSON (without packages <tool_call> Label）
    ...
    def plan_outline(
        ...
        try:
            ...
            # Returns the default outline (3 chapters, asfallback）
    ...
    def _generate_section_react(
        ...
        # Build userprompt - Each completed chapter can be uploaded to a maximum of 4,000 words.
...
class ReportManager:
    ...
    def get_console_log_stream(cls, report_id: str) -> List[str]:
        """
        Get the complete console log (get all at once）
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get the complete Agent log (used to get all at once）
    ...
    def save_section(
        ...
        Args:
            report_id: ReportID
            section_index: Chapter index (starts at 1）
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Convert all level headings（#, ##, ###, ####etc.) converted to bold
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        # ifmarkdown_contentis empty, try starting fromfull_report.mdread
