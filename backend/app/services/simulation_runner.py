...
class SimulationRunState:
    ...
    # Platform completion status (passed detection actions.jsonl in simulation_end event）
...
class SimulationRunner:
    ...
    2. Parse the running log and record the actions of each Agent
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"The simulation configuration does not exist, please call first /prepare interface")
        ...
        try:
            ...
            # Set the child process environment variable to ensure that it is used on Windows UTF-8 coding
            ...
            # use start_new_session=True Create a new process group and ensure that it can pass os.killpg Kill all child processes
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Detection simulation_end event, marking the platform as completed
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            simulation_id: Impersonation ID (used for logging）
            timeout: Timeout to wait for the process to exit (seconds）
        """
        if IS_WINDOWS:
            # Windows: Terminate the process tree using the taskkill command
            # /F = Forced termination, /T = Terminate the process tree (including child processes）
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # skip no agent_id records (non-Agent actions）
    ...
    def get_all_actions(
        ...
        # Read the Twitter action file (automatically set platform to twitter）
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read the Reddit action file (automatically set platform to reddit）
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Note: Configuration files will not be deleted（simulation_config.json）and profile files
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is content that needs to be cleaned up (to avoid empty processes from printing useless logs)）
    ...
    def register_cleanup(cls):
        ...
        # Flask debug mode, only register cleanup in the reloader child process (the process that actually runs the application）
        ...
        # SIGHUP Only exists on Unix systems（macOS/Linux），Windows No
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If the original processor is not callable (e.g. SIG_DFL），then use the default behavior
        ...
        try:
            ...
            logger.warning("Unable to register signal handler (not on main thread), just use atexit")
    ...
    def interview_agent(
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"The simulation environment is not running or has been closed and cannot be executed.Interview: {simulation_id}")
    ...
    def interview_all_agents(
        ...
        if not agent_configs:
            raise ValueError(f"Not available in simulation configurationAgent: {simulation_id}")
