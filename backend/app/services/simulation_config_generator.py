...
class AgentActivityConfig:
    ...
    # emotional tendencies (-1.0arrive1.0，negative to positive)
...
class TimeSimulationConfig:
    ...
    total_simulation_hours: int = 72  # The default simulation is 72 hours (3 days）
    
    # Time of reps per round (simulated minutes）- Default is 60 minutes (1 hour) to speed up time flow
    ...
    # Peak hours (evening19-22point, the time when Chinese people are most active）
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5
    
    # Low time period (early morning0-5, there is almost no activity）
...
class EventConfig:
    """event configuration"""
    # Initial event (triggering event when simulation starts）
...
class PlatformConfig:
    ...
    # The strength of the echo chamber effect (the degree to which similar opinions gather together）
...
class SimulationConfigGenerator:
    ...
    2. Generate Agent configuration in batches (each batch10-20indivual）
    ...
    def generate_config(
        ...
        reasoning_parts.append(f"AgentConfiguration: Successfully generated {len(all_agent_configs)} indivual")
        ...
        logger.info(f"Simulation configuration generation completed: {len(params.agent_configs)} Agent configuration")
...
## Task
Please generate time configurationJSON。
...
- early morning0-5There is almost no activity at the point (activity coefficient0.05）
- Morning6-8Points gradually become active (activity coefficient0.4）
- working hours9-18Points are moderately active (activity coefficient0.7）
- evening19-22The point is the peak period (activity coefficient1.5）
- 23Activity decreases after clicking (activity coefficient0.5）
- General rules: low activity in the early morning, gradual increase in the morning, medium during working hours, peak in the evening
- **important**：The following example values ​​are for reference only. You need to adjust the specific time period according to the nature of the event and the characteristics of the participating groups.
  - For example: the student group peak may be21-23points; media active throughout the day; official agencies only during working hours
  - For example: sudden hot topics may lead to discussions late at night，off_peak_hours Can be shortened appropriately
...
Field description：
- total_simulation_hours (int): Total simulation time，24-168Hours, short for emergencies and long for ongoing topics
- minutes_per_round (int): Duration of each round，30-120minutes, 60 minutes recommended
...
- reasoning (string): Briefly explain why it is configured this way"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format, time configuration must conform to Chinese people’s work and rest habits。"
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse the time configuration results and verifyagents_per_hourThe value does not exceed the total number of agents"""
        # Get original value
...
## Task
Please generate event configurationJSON：
...
Return JSON format (do notmarkdown）：
{{
    ...
    "reasoning": "<Brief description>"
        ...
        system_prompt = "You are an expert in public opinion analysis. Return pure JSON format. Notice poster_type Must exactly match available entity types。"
    ...
    def _assign_initial_post_agents(
        ...
        # Type mapping table (handles the different formats that LLM may output）
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                logger.warning(f"type not found '{poster_type}' of matching Agents, using the one with the highest influence Agent")
                if agent_configs:
...
- **The time is consistent with Chinese people’s schedule**：early morning0-5There is almost no activity at night19-22most active point
...
- **personal**（Student/Person/Alumni）：High activity(0.6-0.9)，Main evening activities(18-23)，Fast response(1-15minute)，low influence(0.8-1.2)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<Active hours list, taking Chinese people’s schedule into consideration>],
        ...
        system_prompt = "You are an expert in analyzing social media behavior. Return pure JSON, the configuration must conform to Chinese people’s work and rest habits。"
        
        try:
            result = self._call_llm_with_retry(prompt, system_prompt)
            llm_configs = {cfg["agent_id"]: cfg for cfg in result.get("agent_configs", [])}
        except Exception as e:
            logger.warning(f"AgentConfiguring batch LLM generation failed: {e}, Generate using rules")
            llm_configs = {}
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent configuration based on rules (Chinese work and rest）"""
