...
class OasisAgentProfile:
    ...
    # optional fields - Redditstyle
    karma: int = 1000
    
    # optional fields - Twitterstyle
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        # Add additional personality information (if any）
        if self.age:
...
class OasisProfileGenerator:
    ...
    1. Call the Zep map search function to get richer context
    ...
    def __init__(
        ...
        if self.zep_api_key:
            try:
                ...
                logger.warning(f"ZepClient initialization failed: {e}")
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        # Must havegraph_idto search
        if not self.graph_id:
            logger.debug(f"Skip Zep retrieval: not setgraph_id")
            return results
        
        comprehensive_query = f"about{entity_name}All information, activities, events, relationships and context of"
        
        def search_edges():
            """search edge(fact/relation）- With retry mechanism"""
            max_retries = 3
            last_exception = None
            delay = 2.0
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"ZepEdge search {attempt + 1} failed: {str(e)[:80]}, Retrying...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"ZepSearch at {max_retries} Failed after attempts: {e}")
            return None
        ...
        def search_nodes():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    last_exception = e
                    if attempt < max_retries - 1:
                        logger.debug(f"ZepNode search No. {attempt + 1} failed: {str(e)[:80]}, Retrying...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"ZepNode search in {max_retries} Failed after attempts: {e}")
            return None
        ...
        try:
            ...
            logger.info(f"ZepMixed search completed: {entity_name}, Get {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")
            
        except concurrent.futures.TimeoutError:
            logger.warning(f"ZepRetrieval timeout ({entity_name})")
        except Exception as e:
            logger.warning(f"ZepRetrieval failed ({entity_name}): {e}")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        include：
        1. Side information about the entity itself (facts）
        2. Details of the associated node
        3. ZepMix the rich information retrieved
        """
        context_parts = []
    ...
    def _generate_profile_with_llm(
        ...
        - group/Organizational entities: Generate representative account settings
        """
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Not setmax_tokens，Let LLM play freely
                )
                
                content = response.choices[0].message.content
                
                # Check if truncated（finish_reasonno'stop'）
                finish_reason = response.choices[0].finish_reason
                if finish_reason == 'length':
                    logger.warning(f"LLMOutput is truncated (attempt {attempt+1}), try to fix...")
                ...
                try:
                    ...
                    logger.warning(f"JSONParsing failed (attempt {attempt+1}): {str(je)[:80]}")
        ...
        logger.warning(f"LLMFailed to generate character（{max_attempts}attempts）: {last_error}, Generate using rules")
        return self._generate_profile_rule_based(
            entity_name, entity_type, entity_summary, entity_attributes
        )
    
    def _fix_truncated_json(self, content: str) -> str:
        """Fix truncated JSON (output wasmax_tokensLimit truncation）"""
        import re
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in social media persona generation. Generate detailed and realistic characters for public opinion simulation,Restore the existing reality to the greatest extent possible. Valid JSON format must be returned, all string values ​​cannot contain unescaped newlines. Use Chinese。"
        return base_prompt
...
2. persona: Detailed character description (2000 words of plain text), must include:
   ...
   - Point of view (attitude towards the topic, possible irritation)/touching content）
...
- Use Chinese (except the gender field which must be in English)male/female）
- Content should be consistent with entity information
- ageMust be a valid integer and gender must be"male"or"female"
"""
...
2. persona: Detailed account setting description (2000 words of plain text), must include:
   - Basic information of the organization (formal name, nature of the organization, establishment background, main functions）
   ...
   - Position and attitude (official stance on core topics, handling of disputes）
   - Special instructions (representative group portrait, operating habits）
   - Institutional memory (an important part of the institution's personality, it should introduce the relationship between the institution and the event, as well as the institution's actions and reactions in the event)）
3. age: Fixed filling in 30 (virtual age of the organization account）
...
- personaIt must be a coherent text description, do not use line breaks
- Use Chinese (except the gender field which must be in English)"other"）
...
- Speeches from institutional accounts must conform to their identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: File path to be written to in real time (if provided, it will be written every time one is generated）
        ...
        # set upgraph_idUsed for Zep retrieval
        if graph_id:
            self.graph_id = graph_id
        ...
        logger.info(f"Start parallel build {total} Agent personas (parallel number: {parallel_count}）...")
        print(f"\n{'='*60}")
        print(f"Start generating Agent profile - common {total} entities, number of parallels: {parallel_count}")
        print(f"{'='*60}\n")
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # Handling newline characters (replaced by spaces in CSV）
                ...
                row = [
                    ...
                    user_char,              # user_char: Complete character set (used internally by LLM）
                    description             # description: Short introduction (external display）
                ]
                writer.writerow(row)
        
        logger.info(f"saved {len(profiles)} Twitter Profile to {file_path} (OASIS CSVFormat)")
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        must contain user_id field, which is OASIS agent_graph.get_agent() The key to matching!
        
        Required fields：
        - user_id: User ID (integer, used to match initial_posts in poster_agent_id）
        - username: username
        - name: display name
        - bio: Introduction
        ...
        logger.info(f"saved {len(profiles)} Reddit Profile to {file_path} (JSONformat, includinguser_idField)")
