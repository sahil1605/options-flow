...
def reset_project(project_id: str):
    """
    Reset project status (used to rebuild the map）
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    ...
    try:
        ...
        project.status = ProjectStatus.ONTOLOGY_GENERATED
        ProjectManager.save_project(project)
        logger.info(f"=== Ontology generation completed === projectID: {project.project_id}")
...
def build_graph():
    """
    Interface 2: According toproject_idBuild a map
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "The project has not generated an ontology yet, please call first /ontology/generate"
            }), 400
