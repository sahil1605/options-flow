...
Step2: ZepEntity reading and filtering, OASIS simulation preparation and running (full automation）
"""
...
def get_graph_entities(graph_id: str):
    ...
    Query parameters：
        ...
        enrich: Whether to obtain relevant side information (defaulttrue）
    """
    try:
...
def create_simulation():
    ...
    Note：max_roundsParameters such as these are intelligently generated by LLM and do not require manual settings.
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "The project has not yet built a map, please call first /api/graph/build"
            }), 400
...
def prepare_simulation():
    ...
    This is a time-consuming operation and the interface will return immediatelytask_id，
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to get the number of entities synchronously (will try again in background task）: {e}")
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "The preparation task has been started, please pass /api/simulation/prepare/status Query progress",
...
def get_prepare_status():
    ...
    try:
        ...
        # if providedsimulation_id，First check whether it is ready
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Preparation has not started yet, please call /api/simulation/prepare start",
                        "already_prepared": False
                    }
                })
            return jsonify({
        ...
        if not task:
            # The task does not exist, but if it doessimulation_id，Check if it is ready
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "The task is completed (preparation work already exists）",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    Get the latest version corresponding to simulation report_id
    
    Traverse the reports directory and find out simulation_id matching report,
    ...
    try:
        ...
        logger.warning(f"Find simulation {simulation_id} The report failed: {e}")
        return None
...
def get_simulation_history():
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get associated report_id（Find the latest version of this simulation report）
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Read the file directly without going through SimulationManager
    - Suitable for real-time viewing during the build process
    - Returns additional metadata (such as file modification time, whether it is being generated, etc.）
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"The simulation configuration does not exist, please call first /prepare interface"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "The configuration file does not exist, please call first /prepare interface"
            }), 404
...
def start_simulation():
    ...
    Request（JSON）：
        {
            ...
            "force": false                         // Optional: Force a restart (will stop the running simulation and clear the logs）
        }
    ...
    About the force parameter：
        ...
        - Configuration files will not be cleaned（simulation_config.json）and profile files
        - Suitable for scenarios where the simulation needs to be re-run

    About enable_graph_memory_update：
        - Once enabled, all Agent activities (posts, comments, likes, etc.) in the simulation will be updated to the Zep graph in real time.
        ...
        - Use batch update mechanism to reduce the number of API calls
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"The simulation is running, please call first /stop The interface is stopped, or using force=true force restart"
                            }), 400
                ...
                logger.info(f"simulation {simulation_id} The preparation work has been completed and the reset state is ready (original state: {state.status.value}）")
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation is not ready, current state: {state.status.value}，Please call first /prepare interface"
                }), 400
        
        # Get the map ID (used for map memory update）
        graph_id = None
...
def get_simulation_actions(simulation_id: str):
    ...
    Query parameters：
        limit: Return quantity (default100）
...
def get_simulation_timeline(simulation_id: str):
    ...
    Query parameters：
        start_round: Starting round (default0）
...
def get_simulation_posts(simulation_id: str):
    ...
    Query parameters：
        ...
        limit: Return quantity (default50）
        offset: offset
...
def interview_agent():
    ...
    try:
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": "platform Parameters can only be 'twitter' or 'reddit'"
            }), 400
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Interview List No.{i+1}Item missing agent_id"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Interview List No.{i+1}Item missing prompt"
                }), 400
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview List No.{i+1}The platform of an item can only be 'twitter' or 'reddit'"
                }), 400
        ...
        logger.error(f"Batch interviews failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def interview_all_agents():
    ...
    Request（JSON）：
        {
            "simulation_id": "sim_xxxx",            // Required, simulateID
            "prompt": "What is your overall opinion on this matter?？",  // Required, interview questions (all agents use the same questions）
