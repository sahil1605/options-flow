    ...
    try:
        ...
        def run_generate():
            try:
                ...
                # Generate a report (passing in a pre-generated report_id）
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "The report generation task has been started, please pass /api/report/generate/status Query progress",
...
def get_generate_status():
    ...
    try:
        ...
        # if providedsimulation_id，First check if there is already a completed report
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_agent_log(report_id: str):
    ...
    Get real-time access to every step in the report generation process, including：
...
def get_console_log(report_id: str):
    ...
    This is related to agent-log The structured JSON log returned by the interface is different.
...
# ============== Tool calling interface (for debugging use）==============
