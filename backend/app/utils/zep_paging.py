...
This module encapsulates automatic page turning logic (including single page retry) and transparently returns the complete list to the caller.。
"""
...
def fetch_all_nodes(
    ...
    """Get graph nodes in pages, return at most max_items bars (default 2000). Each page request comes with a retry。"""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Get all the edges of the graph in pages and return the complete list. Each page request comes with a retry。"""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
