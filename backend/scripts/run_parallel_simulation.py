...
Run Twitter and Reddit simulations simultaneously, reading the same configuration files
...
# ============================================================
# Fix Windows encoding issues: set before all imports UTF-8 coding
...
if sys.platform == 'win32':
    ...
    # Force a default encoding (affects open() Default encoding for functions）
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Package open() function, used by default for text mode UTF-8 encoding
...
# Load the project root directory .env files (containing LLM_API_KEY and other configurations）
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This can be done in actions.jsonl displays the real entity name instead of "Agent_0" Such a code name
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid value last read (use rowid instead of created_at，Because different platforms created_at Different formats）
    ...
    try:
        ...
        # This can be avoided created_at Format difference issue (Twitter uses integers, Reddit uses date and time strings）
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # streamline action_args，Only keep key fields (keep the complete content without truncation)）
            simplified_args = {}
            ...
            # Supplement contextual information (post content, username, etc.）
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # if .env If there is no model name in , use config as a fallback
    if not llm_model:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("API Key configuration is missing, please configure it in the project root directory .env Set in file LLM_API_KEY")
...
async def run_twitter_simulation(
    ...
    Returns:
        PlatformSimulation: Contains env andagent_graphthe result object
    """
    ...
    # Get the Agent real name mapping from the configuration file (using entity_name rather than the default Agent_X）
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # use wait_for Replace sleep, which is responsive shutdown_event
                try:
...
def setup_signal_handlers(loop=None):
    ...
    When a termination signal is received, it is required：
    ...
    def signal_handler(signum, frame):
        ...
        # Don't be direct sys.exit()，Let the asyncio loop exit gracefully and clean up resources
